...
func TestWithSkipFuncAlwaysReturnErr(t *testing.T) {
	defer resetCache()

	//返回err，则GenUID会透传这个错误
	_err := errors.New("any")
	...
	for i := 1; i <= 3; i++ {
		id, err := g.GenUid(context.TODO())
		assert.EqualError(t, err, errors.Wrap(_err, "skipFn").Error())
		assert.Equal(t, uint64(0), id)
	}
}

func TestWithSkipFunc2(t *testing.T) {
	...
	g := NewUidGenerator(new(Locker), new(queuedPool), getCurrMaxUID, WithSkipFunc(skipFn), WithPoolConfig(100))
	for i := 0; i < 23; i++ {
		id, err := g.GenUid(timeoutCtx(time.Millisecond * 20))
		assert.Nil(t, err)
	...
	// 每次进行池填充的都会把池填满，而池size=100，所以这里会跳过 (9976,10077] 范围内符合上述 skipFn 逻辑的id
...
// 并发测试
// 测试结果：   最低耗时  中位数  最高耗时
// 100个并发：  2ms     2.2ms   3.0ms
// 500个并发：  7.9ms   4.3ms   14ms
// 1000个并发： 15ms    19ms    33ms
func TestConcurrencyGenUID(t *testing.T) {
	...
	opts := []Option{
		...
		WithPoolConfig(expectedIdNum), // 默认池大小是100，增加该值有助于提高并发
	}
	...
	t.Logf("low:%s mid:%s high:%s", durationStr[0], durationStr[len(durationStr)/2], durationStr[len(durationStr)-1])
}
